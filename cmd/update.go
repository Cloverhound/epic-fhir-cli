...
var updateCmd = &cobra.Command{
	Use:   "update",
	Short: "Update fhir-cli to the latest version",
...
func fetchLatestVersion() (string, error) {
	resp, err := http.Get("https://api.github.com/repos/Cloverhound/epic-fhir-cli/releases/latest")
	if err != nil {
		return "", err
	}
...
// GoReleaser naming template: epic-fhir-cli_{VERSION}_{OS}_{ARCH}.tar.gz (.zip on Windows).
func archiveURL(version string) string {
	...
	return fmt.Sprintf(
		"https://github.com/Cloverhound/epic-fhir-cli/releases/download/v%s/epic-fhir-cli_%s_%s_%s.%s",
...
func downloadAndReplace(url, binaryPath string) error {
	...
	tmp, err := os.CreateTemp(dir, "fhir-update-*")
	if err != nil {
		if os.IsPermission(err) {
			return fmt.Errorf("permission denied writing to %s — try: sudo fhir-cli update", dir)
		}
	...
	if err := os.Rename(tmpPath, binaryPath); err != nil {
		os.Remove(tmpPath)
		if os.IsPermission(err) {
			return fmt.Errorf("permission denied replacing %s — try: sudo fhir-cli update", binaryPath)
		}
...
// extractFromTarGz extracts the "fhir-cli" binary from a .tar.gz archive.
func extractFromTarGz(data []byte) ([]byte, error) {
	...
	for {
		...
		if filepath.Base(hdr.Name) == "fhir-cli" {
...
// extractFromZip extracts the "fhir-cli.exe" binary from a .zip archive.
func extractFromZip(data []byte) ([]byte, error) {
	...
	for _, f := range zr.File {
		if filepath.Base(f.Name) == "fhir-cli.exe" {
			rc, err := f.Open()
			if err != nil {
				return nil, err
			}
